...
def _redis_ping_burst(host, n_ops, timeout, port=6379, netns_id=None):
    """Pipeline n_ops PING commands to the Redis at host:port.

    Returns (ops_per_sec, avg_latency_ms, failed).
    ...
    try:
        ...
        if pongs == 0:
            return 0.0, timeout * 1000, True
        return pongs / elapsed, elapsed / pongs * 1000, False
    except Exception:  # noqa: BLE001
        return 0.0, timeout * 1000, True


def _start_timeseries_sampler(guest_ip, workload, netns_id=None):  # noqa: ARG001 — workload reserved for future use
    ...
    """
    samples = []          # [(t_rel_s, ops_per_sec, avg_latency_ms, failed)]
    ...
    def _loop():
        while not stop_event.is_set():
            ...
            ops, lat_ms, failed = _redis_ping_burst(
                guest_ip, TIMESERIES_SAMPLE_OPS, timeout=2.0, netns_id=netns_id
            )
            samples.append((round(t_rel, 3), ops, round(lat_ms, 3), failed))
...
def _write_timeseries_csv(handle, workload, mem_size_mib, mode, iteration):
    ...
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["t_ms", "t_rel_s", "throughput", "p99_ms", "failed"])
        for t_rel, ops, lat_ms, failed in handle["samples"]:
            w.writerow([
                round(t_rel * 1000, 1),
                t_rel,
                round(ops, 1),
