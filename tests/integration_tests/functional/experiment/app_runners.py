...
def _run_full_snapshot_app(vm, microvm_factory, mem_size_mib, workload, iteration):
    ...
    if ts_handle:
        ...
        # Collect ~30 more samples showing the paused VM (all zeros), then stop
        # before any blocking SSH/restore operations acquire the sampler thread.
        time.sleep(5)
        ...
        failed_count = sum(1 for s in ts_handle["samples"] if s[3])
