...
def _run_live_bpf_snapshot(vm, microvm_factory, mem_size_mib, workload, iteration):
    """Execute one live-bpf-snapshot experiment run. Returns the result row dict."""
    row = {
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "mem_size_mib": mem_size_mib,
        "workload": workload,
    ...
    if workload != "idle":
        ...
        row["service_interruption_ms"] = round(
            (row.get("phase1_us", 0) + row.get("freeze_us", 0)) / 1000, 2
        )
    else:
        row["workload_during_mibs"] = 0
        row["workload_degradation_pct"] = 0
